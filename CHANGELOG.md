...
### Adicionado
- Nova aba **Avaliador Crítico** na dashboard — agente independente
  configurado para avaliar o projecto de forma rigorosa e honesta,
  como um professor universitário experiente faria.
- 7 botões de acesso rápido no avaliador: avaliação global, pontos
  fracos, arquitectura, conformidade EU AI Act, nota, perguntas
  difíceis e qualidade das skills.
- Sistema de retry automático no avaliador em caso de rate limit.
- Skills melhoradas com tabelas de reutilização, referências académicas
  adicionais e argumentação de inovação mais explícita.
...
### Corrigido
- Lógica de verificação ética corrigida — o badge vermelho disparava
  erradamente quando o agente mencionava palavras como "fraude" ou
  "branqueamento" nas suas respostas explicativas. A verificação
  passou a analisar apenas a mensagem do utilizador, não a resposta
  do agente.
- Adicionado retry automático em caso de rate limit da API Groq — o
  sistema aguarda o tempo necessário e reenvia o pedido sem
...
- Subagente de Psicologia Comportamental implementado — detecta
  enviesamentos cognitivos e adapta o tom da comunicação.
